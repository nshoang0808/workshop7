...
/**
  * Add a new comment to a post (statusupdate)
  */
  function postComment(user, feedItemId, contents, callback) {
    // Get the current UNIX time.
    var time = new Date().getTime();
    // The new status update. The database will assign the ID for us.
    var newComment = {
    ...
    db.collection('feedItems').updateOne({ _id: feedItemId},
      ...
      db.collection('feedItems').findOne({ _id: feedItemId }, function(err, feedItem) {
        return callback(null, feedItem.comments.length-1);
      })
    });
  }
  ...
  app.post('/feeditem/:feeditemid/comments', validate({ body: CommentSchema }), function(req, res) {
    ...
    var feedItemId = new ObjectID(req.params.feeditemid);
    if (fromUser === author) {
      postComment(new ObjectID(fromUser), feedItemId, comment.contents, function(err, result) {
        if (err) {
          // A database error happened.
          // 500: Internal error.
          res.status(500).send("A database error occurred: " + err);
          ...
          res.status(201);
          res.set('Location', '/feeditem/' + feedItemId + "/comments/" + result);
          getFeedItem(feedItemId, function(err, feedItem) {
            if (err) {
              return sendDatabaseError(res, err);
            }
  ...
  app.delete('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
    ...
    if (fromUser === userId) {
      db.collection('feedItems').findOne({ _id: feedItemId }, function(err, feedItem) {
        ...
        if (feedItem.comments[commentIdx].likeCounter.indexOf(userId) !== -1) {
          feedItem.comments[commentIdx].likeCounter.splice(userId, 1);
        }
        resolveUserObjects(feedItem.comments[commentIdx].likeCounter, function(err, userMap) {
          if (err) {
            return sendDatabaseError(res, err);
